...
public enum ServiceCollectionDatasetCollectionSetAccess {
	...
	GetListOfDatasetCollectionIDsSet {
		...
		public JsonObject process(JsonObject json) {
			...
			JsonObject address = DatasetCollectionIDManagement.getCollectionOfDatasetCollectionIDsSetAddress(maintainer);
			...
			if(json.get(ClassLabelConstants.DatasetCollectionsSetLabel) != null) {
				setofprops = CreateDocumentTemplate.createTemplate("dataset:SetOfPropertyValueQueryPairs");
				String label = json.get(ClassLabelConstants.DatasetCollectionsSetLabel).getAsString();
				JsonArray props = new JsonArray();
				JsonObject prop1 = CreateDocumentTemplate.createTemplate("dataset:PropertyValueQueryPair");
				prop1.addProperty(ClassLabelConstants.DatabaseObjectType, ClassLabelConstants.DatasetCollectionsSetLabel);
			...
			JsonObject response = ReadFirestoreInformation.readFirestoreCollection(setofprops, address);
			if(response.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean() ) {
				String docS = response.get(ClassLabelConstants.ServiceResponseMessage).getAsString();
				...
				response = DatabaseServicesBase.standardServiceResponse(document, "Success read " + arr.size() + " sets", catalog);
			} else {
				response = DatabaseServicesBase.standardErrorResponse(document, "Error: in reading sets", null);
			}
			return response;
		}
	},
	ReadInDatasetWithDatasetCollection {
		...
		public JsonObject process(JsonObject json) {
			...
			if(identifier != null) {
				if(idsset.get(identifier) != null) {
					...
					JsonObject collectionid = DatasetCollectionIDManagement.firebaseIDOfCollection(collectioninfo);
					...
					response = DatabaseServicesBase.standardErrorResponse(document, classname + 
							"(" + identifier + ")  not is dataset collection: " 
							+ label, null);
				}
			} else {
				response = DatabaseServicesBase.standardErrorResponse(document, "Error: Identifer for " + classname + " not found", null);
			}
			return response;
		}
		...
		public JsonObject process(JsonObject json) {
			...
			if(response.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean() ) {
				JsonObject idsobject = response.get(ClassLabelConstants.SimpleCatalogObject).getAsJsonObject();
				JsonArray ids = idsobject.get(ClassLabelConstants.SimpleCatalogObject).getAsJsonArray();
				if(ids.size() > 0) {
					if(ids.size() == 1) {
						JsonObject collectionids = ids.get(0).getAsJsonObject();
						...
						if(readresponse.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean() ) {
							...
							MessageConstructor.combineBodyIntoDocument(document, docS);
							response = DatabaseServicesBase.standardServiceResponse(document, "Succcesful Read of objects", objects);
						} else {
							String docS = readresponse.get(ClassLabelConstants.ServiceResponseMessage).getAsString();
							MessageConstructor.combineBodyIntoDocument(document, docS);
							Element body = MessageConstructor.isolateBody(document);
						...
						response = DatabaseServicesBase.standardErrorResponse(document, "More than one collection set found", null);
					}
				} else {
					response = DatabaseServicesBase.standardErrorResponse(document, "No collection set found", null);
