...
public class FindDisassociationEnergyCollection {
	...
	public static ArrayList<DisassociationEnergyWithAtomCounts> findDisassociationEnergy(String maintainer, String dataset) {
	...
	public static ArrayList<DisassociationEnergyWithAtomCounts> findDisassociationEnergy(JsonArray arr) {
		...
		if (arr != null) {
			for(int i=0;i<arr.size();i++) {
				...
				JsonObject structure = energy.get(ClassLabelConstants.JThermodynamics2DSpeciesStructure).getAsJsonObject();
				...
				try {
					...
					JsonObject parameter = energy.get(ClassLabelConstants.JThermodynamicDisassociationEnergy).getAsJsonObject();
					...
					JsonObject recordid = energy.get(ClassLabelConstants.DatabaseCollectionOfCurrentClass).getAsJsonObject();
					String source = JsonObjectUtilities.toString(recordid);
					DisassociationEnergyWithAtomCounts disassociation = new DisassociationEnergyWithAtomCounts(molecule, source, value, uncertainty);
