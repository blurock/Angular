        ...
        JsonObject process(JsonObject event, JsonObject prerequisites, JsonObject info) {
            ...
            if (response.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean()) {
                ...
                if (arr.size() > 0) {
                    ...
                    response = DatabaseServicesBase.standardErrorResponse(errdoc, "Error: No catalog objects generated",
                            null);
                }
            } else {
                Document errdoc = MessageConstructor.startDocument("Error:InitialReadInOfRepositoryFile");
        ...
        if (response.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean()) {
            if (!response.get(ClassLabelConstants.SimpleCatalogObject).isJsonNull()) {
                ...
                if (arr.size() > 0) {
                    ...
                    /*
                     * JsonObject rdfresponse = GenerateAndWriteRDFForObject.generate(event); if
                     * (rdfresponse.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean()
                     * ) { String rdfmessage =
                     * rdfresponse.get(ClassLabelConstants.ServiceResponseMessage).getAsString();
                     * MessageConstructor.combineBodyIntoDocument(document, rdfmessage); response =
                     * DatabaseServicesBase.standardServiceResponse(document, "Success: " +
                     * transaction, event); } else { response =
                     * DatabaseServicesBase.standardErrorResponse(document, rdfresponse, event); }
                     */
                } else {
