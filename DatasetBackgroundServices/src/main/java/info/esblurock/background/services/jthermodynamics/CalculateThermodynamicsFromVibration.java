...
public class CalculateThermodynamicsFromVibration {
	...
	private static JsonObject convertToThermodynamicContribution(JsonObject countdiff, JsonObject info, Element table) {
		...
		double symmetry = Double.parseDouble(symmetryS);
       	...
       	BensonThermodynamicBase benson = CalculateVibrationalCorrectionUtilities.contribution(title, matches, frequency, symmetry);
	...
	public static JsonArray computeVibrationalMatchCounts(JsonArray structures, IAtomContainer molecule) {
		...
		for(int i=0;  i < structures.size() ; i++) {
			...
			System.out.println("computeVibrationalMatchCounts   ------------------------------------------------------");
			...
			JsonObject structureinfo = vibstructure.get(ClassLabelConstants.JThermodynamics2DSpeciesStructure).getAsJsonObject();
			...
			try {
				...
				String vibsymmetry = vibstructure.get(ClassLabelConstants.StructureVibrationalFrequencySymmetry).getAsString();
	...
	private static JsonObject findInArray(JsonObject obj, JsonArray arr) {
		...
		while(i<arr.size() && ans == null) {
			...
			//JsonObject arrstruct = arrobj.get(ClassLabelConstants.JThermodynamics2DSpeciesStructure).getAsJsonObject();
			//String arrname = arrstruct.get(ClassLabelConstants.JThermodynamicsStructureName).getAsString();
			i++;
